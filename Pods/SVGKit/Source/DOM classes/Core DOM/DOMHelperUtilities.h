...
@class SVGKNode, NodeList, Element; // avoiding #import here, to avoid C header loop problems.
...
/*! This useful method provides both the DOM level 1 and the DOM level 2 implementations of searching the tree for a node - because THEY ARE DIFFERENT
 yet very similar
 */
+(void) privateGetElementsByName:(NSString*) name inNamespace:(NSString*) namespaceURI childrenOfElement:(SVGKNode*) parent addToList:(NodeList*) accumulator;
...
+(Element*) privateGetElementById:(NSString*) idValue childrenOfElement:(SVGKNode*) parent;
